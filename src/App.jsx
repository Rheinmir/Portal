...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim() || !formData.url.trim()) {
      alert('Tên app và URL là bắt buộc');
      return;
    }
  ...
  const handleExportData = () => {
    const exportData = {
      ...
      shortcuts: shortcuts.map(({ id, ...rest }) => rest),
      labels: Object.entries(labelColors).map(([name, color_class]) => ({
        name,
        color_class
      }))
    };
    ...
    downloadAnchorNode.setAttribute(
      'download',
      `backup_sqlite_${new Date().toISOString().slice(0, 10)}.json`
    );
  ...
  const uniqueChildren = useMemo(
    () =>
      [...new Set(shortcuts.map((s) => s.child_label).filter(Boolean))].sort(),
    [shortcuts]
  );
  ...
  const filteredShortcuts = useMemo(() => {
    ...
    return shortcuts.filter((item) => {
      ...
      const matchesParent = activeParentFilter
        ? item.parent_label === activeParentFilter
        : true;
      const matchesChild = activeChildFilter
        ? item.child_label === activeChildFilter
        : true;
  ...
  const getAppStyle = (item) => {
    const color = labelColors[item.parent_label] || '#0A1A2F';
    return getGradientStyle(color);
  };

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F4F4F4] text-[#5A5A5A]">
        <div className="flex flex-col items-center gap-2">
          <Activity className="w-8 h-8 animate-spin mx-auto mb-1" />
      ...
      <div className="sticky top-0 z-30 w-full flex flex-col items-center pt-8 px-4 pointer-events-none gap-2">
        <div className="flex items-center gap-2 pointer-events-auto w-full max-w-sm">
          <div className="relative group w-full transition-all duration-300">
            ...
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2 border-none rounded-full bg-[#D8D8D8]/40 text-[#2C2C2C] focus:bg-white focus:ring-2 focus:ring-[#009FB8]/50 shadow-sm backdrop-blur-sm transition-all text-sm"
                ...
                <button
                  ...
                  onClick={() =>
                    setActiveParentFilter(
                      activeParentFilter === label ? null : label
                    )
                  }
                  className={`px-3 py-1 text-xs rounded-full border flex items-center gap-1 transition-all ${
                    activeParentFilter === label
                      ? 'text-white border-transparent ring-2 ring-[#009FB8]'
                  ...
                  <button
                    ...
                    onClick={() =>
                      setActiveChildFilter(
                        activeChildFilter === label ? null : label
                      )
                    }
                    className={`px-2 py-0.5 text-[10px] rounded-md border flex items-center gap-1 transition-all ${
                      activeChildFilter === label
                        ? 'bg-[#009FB8] text-white'
                        : 'bg-[#F4F4F4] text-[#5A5A5A]'
                    }`}
                    style={
                      activeChildFilter === label && labelColors[label]
                        ? {
                            background: labelColors[label],
                            color: 'white',
        ...
        <div className="fixed top-0 right-0 p-4 z-40 pointer-events-auto opacity-0 hover:opacity-100 transition-opacity duration-300">
          {isAdmin ? (
            <div className="flex flex-col gap-2 bg-white/50 backdrop-blur-md p-2 rounded-2xl border border-white/20 shadow-lg">
              <div className="flex gap-2">
                <button
                  ...
                  className="p-2 bg-[#0F2F55] text-white rounded-full hover:scale-110 transition-transform"
                >
                  <Plus size={18} />
                </button>
                <button
                  onClick={handleExportData}
                  className="p-2 bg-white text-[#0A1A2F] rounded-full hover:scale-110 transition-transform border border-[#D6E4F5]"
                >
                  <Download size={18} />
                </button>
                <button
                  onClick={() => importInputRef.current?.click()}
                  className="p-2 bg-white text-[#009FB8] rounded-full hover:scale-110 transition-transform border border-[#D6E4F5]"
                >
                  <FileUp size={18} />
                </button>
                <input
              ...
              <button
                ...
                className="p-2 w-full bg-[#BE123C]/10 text-[#BE123C] rounded-xl text-xs font-bold flex items-center justify-center gap-1"
              >
            ...
            <button
              ...
              className="p-2 bg-white text-[#A0A0A0] hover:text-[#0F2F55] rounded-full shadow-lg border border-[#D8D8D8] hover:scale-105 transition-transform"
            >
              <Settings size={20} />
            </button>
          )}
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20 pt-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-8 gap-y-10 justify-items-center">
          {filteredShortcuts.map((item) => (
            <div
              key={item.id}
              className="group relative flex flex-col items-center w-28 cursor-pointer active:scale-95 transition-transform"
            ...
            >
              <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 scale-90">
                <button
                  ...
                  className="p-1.5 bg-white text-[#A0A0A0] hover:text-[#009FB8] rounded-full shadow-sm border border-[#D8D8D8]"
                >
                  {copiedId === item.id ? (
                    <Check size={12} className="text-[#009FB8]" />
                  ) : (
                    <Copy size={12} />
                  )}
                </button>
                {isAdmin && (
                  <>
                    <button
                      ...
                      className="p-1.5 bg-white text-[#A0A0A0] hover:text-[#0A1A2F] rounded-full shadow-sm border border-[#D8D8D8] ml-1"
                    >
                      <Pencil size={12} />
                    </button>
                    <button
              ...
              <div
                className="w-20 h-20 rounded-[1.4rem] flex items-center justify-center text-white text-3xl font-bold shadow-md hover:shadow-lg transition-all mb-2 overflow-hidden relative"
                style={getAppStyle(item)}
              >
                {item.icon_url ? (
                  <img
                    src={item.icon_url}
              ...
              <span className="text-sm font-medium text-[#5A5A5A] text-center truncate w-32 px-1 group-hover:text-[#0A1A2F]">
                {item.name}
              </span>
              <div className="flex flex-wrap justify-center gap-1 mt-1 px-1">
                {item.parent_label && (
                  <span
                    className="text-[9px] px-1.5 py-0.5 rounded-full text-white truncate max-w-[70px]"
                    style={{
                      background:
                ...
                {item.child_label && (
                  <span
                    className="text-[9px] px-1.5 py-0.5 rounded-full border border-[#D8D8D8] text-[#5A5A5A] truncate max-w-[70px]"
                    style={{
                      borderColor:
                        labelColors[item.child_label] || '#E5E7EB'
                    }}
                  >
                    {item.child_label}
                  </span>
          ...
          {isAdmin && (
            ...
            >
              <div className="w-20 h-20 rounded-[1.4rem] bg-[#D8D8D8]/40 border-2 border-dashed border-[#A0A0A0] flex items-center justify-center text-[#A0A0A0] group-hover:text-[#0F2F55] transition-all mb-3">
                <Plus size={32} />
              </div>
              <span className="text-sm font-medium text-[#A0A0A0] group-hover:text-[#0F2F55]">
                Thêm App
              </span>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-[#0A1A2F]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xs p-6 border border-[#D8D8D8]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-[#0A1A2F]">Admin Login</h3>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <input
                ...
                className="w-full px-4 py-2 bg-[#F4F4F4] rounded-lg"
              ...
              <button className="w-full py-2 bg-[#0F2F55] text-white rounded-lg">
                Login
              </button>
            </form>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold">
                {formData.id ? 'Sửa Ứng Dụng' : 'Thêm Ứng Dụng'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <input
                type="text"
                placeholder="Tên App"
                className="w-full px-4 py-3 bg-[#F4F4F4] rounded-xl"
                value={formData.name}
                onChange={(e) =>
              ...
              <div
                ...
                className="w-full px-4 py-3 bg-[#F4F4F4] rounded-xl cursor-pointer flex items-center gap-2"
              >
                {formData.icon_url ? (
                  <img
              ...
              <div className="border-t border-[#D8D8D8] pt-3 space-y-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <input
                      type="text"
                      placeholder="Nhóm lớn (VD: Work)"
                      className="flex-1 px-3 py-2 bg-[#F4F4F4] border border-[#D8D8D8] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#009FB8]"
                      value={formData.parent_label}
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          parent_label: e.target.value
                        }))
                      }
                    />
                  </div>
                    ...
                    {COLOR_PRESETS.map((c) => (
                      <div
                        ...
                        className={`w-6 h-6 rounded-full cursor-pointer border border-gray-300 ${
                          formData.parent_color === c ? 'ring-2 ring-black' : ''
                        }`}
                        style={{ background: c }}
                      />
                    ))}
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <input
                      type="text"
                      placeholder="Nhóm con (VD: Dev)"
                      className="flex-1 px-3 py-2 bg-[#F4F4F4] border border-[#D8D8D8] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#009FB8]"
                      value={formData.child_label}
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          child_label: e.target.value
                        }))
                      }
                    />
                  </div>
                    ...
                    {COLOR_PRESETS.map((c) => (
                      <div
                        ...
                        className={`w-5 h-5 rounded-full cursor-pointer border border-gray-300 ${
                          formData.child_color === c ? 'ring-2 ring-black' : ''
                        }`}
                        style={{ background: c }}
                      />
                    ))}
              ...
              <button className="w-full py-3 bg-[#0F2F55] text-white rounded-xl">
                Lưu
              </button>
            </form>
          </div>
