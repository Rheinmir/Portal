...
const DEFAULT_LIGHT_TEXT='#2C2C2C',DEFAULT_DARK_TEXT='#E2E8F0';
...
const isVideoSource=s=>typeof s==='string'&&/\.(mp4|webm|ogg)$/i.test(s);

export default function App(){
  const[shortcuts,setShortcuts]=useState([]),
        [labelColors,setLabelColors]=useState({}),
        [loading,setLoading]=useState(true),
        [darkMode,setDarkMode]=useState(()=>localStorage.getItem('darkMode')==='true'),
  ...
  const[lightTextColor,setLightTextColor]=useState(()=>localStorage.getItem('custom_text_light')||DEFAULT_LIGHT_TEXT),
        [darkTextColor,setDarkTextColor]=useState(()=>localStorage.getItem('custom_text_dark')||DEFAULT_DARK_TEXT);
  const[formData,setFormData]=useState({id:null,name:'',url:'',icon_url:'',parent_label:'',parent_color:COLOR_PRESETS[0],child_label:'',child_color:COLOR_PRESETS[1],isLocal:false}),
        ...
        [tenant,setTenant]=useState(()=>normalizeTenant(localStorage.getItem('tenant'))),
  ...
  const applyBackgroundSource=(src,isFromServer=false)=>{
    if(!src){
      setBgImage(null);
      setBgVideo(null);
      if(isFromServer)setServerBg(null);
      return;
    }
  ...
  const fetchData=async()=>{
    try{
      ...
      const ls=JSON.parse(localStorage.getItem('local_shortcuts')||'[]').map(s=>({
        ...s,
        isLocal:true,
        child_label:(s.child_label||'').includes('Personal')?s.child_label:(s.child_label?(s.child_label+', Personal'):'Personal')
      }));
      setShortcuts([...ss,...ls]);
      ...
      if(serverVer>localVer){
        ...
        const defBg=c.default_background||null;
        applyBackgroundSource(defBg,true);
        ...
        const cfgBg=c.default_background||null;
        setServerBg(cfgBg||null);
        const localBg=localStorage.getItem('custom_bg')||cfgBg||null;
        applyBackgroundSource(localBg,false);
  ...
  const handleBgUpload=e=>{
    ...
    r.onload=ev=>{
      ...
      setBgImage(b);
      if(isAdmin){
        if(confirm("Lưu mặc định server?")){
          saveConfig('default_background',b);
          alert("Đã lưu server!")
        }else{
          localStorage.setItem('custom_bg',b)
        }
      }else localStorage.setItem('custom_bg',b)
    };
    r.readAsDataURL(f)
  };
  ...
  const applyBgUrl=()=>{
    ...
    if(isAdmin){
      if(confirm("Lưu làm nền mặc định server?")){
  ...
  const handleResetBg=()=>{
    localStorage.removeItem('custom_bg');
    if(serverBg&&isVideoSource(serverBg)){
      setBgVideo(serverBg);
      setBgImage(null);
    }else{
  ...
  const handleSubmit=async e=>{
    ...
    if(isLocal){
      ...
      try{
        const r=await fetch(formData.id?`/api/shortcuts/${formData.id}`:'/api/shortcuts',{
  ...
  const handleImageUpload=e=>{
    ...
    if(f){
      const r=new FileReader();
      r.onload=ev=>setFormData(p=>({...p,icon_url:ev.target.result}));
      r.readAsDataURL(f)
    }
  };
  ...
  const handleForceSync=async()=>{
    ...
    if(confirm("Cập nhật cấu hình lên Server và ép Client tải lại?")){
      try{
        ...
        if(bgVideo)p.default_background=bgVideo;
        else if(bgImage)p.default_background=bgImage;
  ...
  const handleDrop=(e,targetId)=>{
    ...
    setClientOrder(prev=>{
      ...
      let current=prev&&prev.length?prev.filter(id=>baseIds.includes(id)):baseIds.slice();
      baseIds.forEach(id=>{if(!current.includes(id))current.push(id)});
  ...
  const filteredShortcuts=useMemo(()=>{
    let r=shortcuts.filter(i=>{
      ...
      const m=(!t||i.name.toLowerCase().includes(t))&&(!activeParentFilter||i.parent_label===activeParentFilter);
      if(!m)return false;
  ...
  const inputClass=darkMode?'bg-gray-800 border-gray-700':'bg-white border-[#D8D8D8]',
        modalClass=darkMode?'bg-gray-900 border-gray-700':'bg-white border-[#D8D8D8]';
  ...
  return (
    <div className={`min-h-screen font-light transition-all duration-300 bg-cover bg-center bg-no-repeat bg-fixed ${bgClass}`} style={{backgroundImage:bgImage?`url(${bgImage})`:'none',color:currentTextColor}}>
      {bgVideo&&(
        <video className="fixed inset-0 w-full h-full object-cover -z-10" src={bgVideo} autoPlay loop muted playsInline/>
      )}
      <div className="min-h-screen w-full transition-colors duration-300" style={{backgroundColor:(bgImage||bgVideo)?(darkMode?`rgba(0,0,0,${overlayOpacity})`:`rgba(255,255,255,${overlayOpacity})`):''}}>
          ...
          <div className="pointer-events-auto w-full max-w-2xl mx-auto flex items-center justify-center gap-3">
            <div className="flex-1 flex items-center gap-2 min-w-0">
               <div className="relative group w-full max-w-lg mx-auto transition-all">
                  ...
                  <input type="text" className={`block w-full pl-10 pr-3 py-2 border rounded-full text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-[#009FB8] ${inputClass} ${(bgImage||bgVideo)?'bg-opacity-60 backdrop-blur-md':'bg-opacity-60'}`} style={{color:currentTextColor}} placeholder="Tìm kiếm..." value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} />
               </div>
               <button onClick={()=>setShowFilterPanel(!showFilterPanel)} className={`p-2 rounded-full shadow-sm border ${inputClass} ${(bgImage||bgVideo)?'bg-opacity-80':''}`}><Filter size={18}/></button>
          ...
          {showFilterPanel&&(
            <div className={`pointer-events-auto w-full max-w-5xl mx-auto rounded-2xl p-3 shadow-lg flex flex-col gap-2 border ${modalClass} bg-opacity-95 backdrop-blur-md`}>
              <div className="flex flex-wrap gap-2">
                ...
                {uniqueParents.map(l=>(
                  <button key={l} onClick={()=>setActiveParentFilter(l)} className={`px-3 py-1 text-xs rounded-full border ${activeParentFilter===l?'ring-2 ring-[#009FB8]':''}`} style={{background:labelColors[l],color:getContrastYIQ(labelColors[l])}}>{l}</button>
                ))}
              </div>
              {uniqueChildren.length>0&&(
                <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-500/20">
                  ...
                  {uniqueChildren.map(l=>(
                    <button key={l} onClick={()=>setActiveChildFilter(activeChildFilter===l?null:l)} className={`px-2 py-0.5 text-[10px] rounded-full border ${activeChildFilter===l?'bg-[#009FB8] text-white':''}`} style={activeChildFilter===l&&labelColors[l]?{background:labelColors[l],borderColor:labelColors[l],color:getContrastYIQ(labelColors[l])}:{}}>{l}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        <div ref={gridWrapperRef} className="max-w-7xl mx-auto px-6 pb-32 pt-8 min-h-[60vh]" onWheel={e=>{const d=Math.abs(e.deltaX)>Math.abs(e.deltaY)?e.deltaX:e.deltaY;if(Math.abs(d)>40){if(d>0)goNext();else goPrev()}}} onTouchStart={e=>setTouchStartX(e.touches[0].clientX)} onTouchEnd={e=>{if(touchStartX===null)return;const d=e.changedTouches[0].clientX-touchStartX;if(Math.abs(d)>50){if(d<0)goNext();else goPrev()}setTouchStartX(null)}}>
          ...
          {pagedShortcuts.map(i=>(
            <div key={i.id} data-card draggable onDragStart={e=>handleDragStart(e,i.id)} onDragOver={handleDragOver} onDrop={e=>handleDrop(e,i.id)} onDragEnd={handleDragEnd} className={`group relative flex flex-col items-center w-full max-w-[100px] cursor-pointer active:scale-95 transition-transform ${draggingId===i.id?'opacity-50 scale-90':''}`} onClick={()=>handleLinkClick(i.id,i.url)}>
              <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 scale-90">
                <button onClick={e=>{e.stopPropagation();setCopiedId(i.id);navigator.clipboard.writeText(i.url);setTimeout(()=>setCopiedId(null),1000)}} className={`p-1.5 rounded-full shadow-sm border ${cardClass} bg-opacity-90`}>{copiedId===i.id?<Check size={12} className="text-green-500"/>:<Copy size={12}/>}</button>
                {(isAdmin||i.isLocal)&&(
                  <>
                    <button onClick={e=>handleEdit(i,e)} className={`p-1.5 rounded-full shadow-sm border ml-1 ${cardClass}`}><Pencil size={12}/></button>
                    <button onClick={e=>{e.stopPropagation();handleDelete(i.id)}} className={`p-1.5 rounded-full shadow-sm border ml-1 ${cardClass}`}><Trash2 size={12}/></button>
                  </>
                )}
              </div>
              ...
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-md hover:shadow-lg transition-all mb-2 overflow-hidden relative backdrop-blur-sm ${i.icon_url?'bg-transparent':'bg-gradient-to-br bg-opacity-90'}`} style={i.icon_url?{}:getGradientStyle(labelColors[i.parent_label]||'#0A1A2F')}>
                {i.icon_url?<img src={i.icon_url} className="w-full h-full object-contain"/>:<span>{i.name.charAt(0).toUpperCase()}</span>}
              </div>
              <span className="text-xs text-center truncate w-full px-1 leading-tight font-light" style={{textShadow:(bgImage||bgVideo)?'0 1px 2px rgba(0,0,0,0.5)':'none'}}>{i.name}</span>
              <div className="flex flex-wrap justify-center gap-1 mt-1 px-1">
                {i.parent_label&&(
                  <span className="text-[8px] px-1 py-0.5 rounded-full text-white truncate max-w-[60px] shadow-sm mb-0.5" style={{background:labelColors[i.parent_label]||'#9CA3AF',color:getContrastYIQ(labelColors[i.parent_label]||'#9CA3AF')}}>{i.parent_label}</span>
                )}
                {(i.child_label||'').split(',').filter(Boolean).map(t=>(
                  <span key={t} className={`text-[8px] px-1 py-0.5 rounded-full border truncate max-w-[60px] bg-white/50 backdrop-blur-sm ${darkMode?'border-gray-600':'border-gray-300'}`} style={{borderColor:labelColors[t?.trim()],color:labelColors[t?.trim()]||(darkMode?'#ddd':'#333')}}>{t.trim()}</span>
                ))}
              </div>
            </div>
          ))}
        ...
        {totalPages>1&&(
          <div className="flex justify-center mt-6 mb-4 gap-2">
            {Array.from({length:totalPages}).map((_,i)=>(
              <button key={i} onClick={()=>setCurrentPage(i)} className={`w-2.5 h-2.5 rounded-full transition-all duration-200 border ${i===currentPage?(darkMode?'bg-white border-white scale-110':'bg-gray-800 border-gray-800 scale-110'):(darkMode?'border-white/40 bg-white/10':'border-gray-400/40 bg-gray-500/10')}`}/>
            ))}
          </div>
        )}
        </div>
        ...
        <div className="fixed bottom-6 right-6 z-50 pointer-events-auto opacity-0 hover:opacity-100 transition-opacity duration-300">
          <div className="group/menu flex items-center justify-end gap-2 p-2 rounded-full hover:bg-white/20 hover:backdrop-blur-md transition-all">
            <div className="flex items-center gap-2">{(bgImage||bgVideo)&&<div className="flex items-center gap-1 mr-2 bg-black/40 rounded-full px-2 py-1 backdrop-blur-sm"><span className="text-[10px] text-white/90 font-bold">BG</span><input type="range" min="0" max="0.9" step="0.1" value={overlayOpacity} onChange={e=>{const v=parseFloat(e.target.value);setOverlayOpacity(v);localStorage.setItem('overlayOpacity',v);if(isAdmin)saveConfig('overlay_opacity',v)}} className="w-16 h-1 accent-[#009FB8] cursor-pointer"/></div>}
            <button onClick={()=>setDarkMode(!darkMode)} className={`p-2 rounded-full border shadow-sm ${inputClass} ${(bgImage||bgVideo)?'bg-opacity-80':''}`}>{darkMode?<Sun size={18} className="text-yellow-400"/>:<Moon size={18} className="text-gray-600"/>}</button>
            <div className={`flex items-center gap-1 p-1 rounded-full border shadow-lg ${inputClass} bg-opacity-80 backdrop-blur`}>
              <div className="flex flex-col gap-0.5 mr-1 border-r border-gray-400/30 pr-1">
                <div className="flex items-center gap-1" title="Text Light">
                  <input type="color" value={lightTextColor} onChange={e=>handleTextColorChange('light',e.target.value)} className="w-4 h-4 p-0 border-none bg-transparent cursor-pointer"/>
                </div>
                <div className="flex items-center gap-1" title="Text Dark">
                  <input type="color" value={darkTextColor} onChange={e=>handleTextColorChange('dark',e.target.value)} className="w-4 h-4 p-0 border-none bg-transparent cursor-pointer"/>
                </div>
              </div>
              ...
              <div className="hidden sm:flex items-center gap-1 ml-1">
                  <input type="text" placeholder="Link ảnh/GIF/MP4" className={`px-2 py-1 text-[11px] rounded-full border max-w-[160px] ${inputClass}`} value={bgUrlInput} onChange={e=>setBgUrlInput(e.target.value)}/>
                  <button type="button" onClick={applyBgUrl} className="px-2 py-1 text-[11px] rounded-full border border-gray-400/50 hover:bg-gray-200 dark:hover:bg-gray-700">Set</button>
              </div>
              {isAdmin&&(<>
            ...
            <div className={`rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 ${modalClass}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 rounded-xl border border-gray-500/20">
                  ...
                  {insightsData.topApps.length===0?(
                    <p className="text-xs opacity-60">Chưa có dữ liệu.</p>
                  ):(
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={insightsData.topApps}>
                          <XAxis dataKey="name" tick={{fontSize:10}} tickLine={false} axisLine={false} interval={0} angle={-35} textAnchor="end" height={60}/>
                ...
                <div className="p-4 rounded-xl border border-gray-500/20">
                  <h4 className="text-sm font-bold mb-4 opacity-80">Lượt click theo giờ</h4>
                  {insightsData.hourly.length===0?(
                    <p className="text-xs opacity-60">Chưa có dữ liệu.</p>
                  ):(
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={insightsData.hourly.map(h=>({hour:`${h.h}h`,count:h.count}))}>
                          <XAxis dataKey="hour" tick={{fontSize:9}} tickLine={false} axisLine={false} interval={1}/>
                          <YAxis tick={{fontSize:10}}/>
                          <Tooltip cursor={{fill:'rgba(148,163,184,0.15)'}}/>
                          <Bar dataKey="count" radius={[3,3,0,0]} fill="#10B981"/>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className={`rounded-2xl shadow-2xl w-full max-w-xs p-6 border ${modalClass}`}>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <input type="text" placeholder="User" className={`w-full px-4 py-2 rounded-lg text-sm border ${inputClass}`} value={loginCreds.username} onChange={e=>setLoginCreds({...loginCreds,username:e.target.value})}/>
                <input type="password" placeholder="Pass" className={`w-full px-4 py-2 rounded-lg text-sm border ${inputClass}`} value={loginCreds.password} onChange={e=>setLoginCreds({...loginCreds,password:e.target.value})}/>
                {loginError&&<p className="text-red-500 text-xs">{loginError}</p>}
                <button className="w-full py-2 bg-[#0F2F55] text-white rounded-lg hover:bg-opacity-90">Login</button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className={`rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto border ${modalClass}`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold">{formData.id?'Sửa':'Thêm'} App</h3>
                <button onClick={()=>setShowAddModal(false)}><X size={20}/></button>
              </div>
              <form onSubmit={handleSubmit} className="space-y-3">
                <input type="text" placeholder="Tên" className={`w-full px-4 py-3 rounded-xl text-sm border ${inputClass}`} value={formData.name} onChange={e=>setFormData({...formData,name:e.target.value})} required/>
                ...
                <div className={`border-t pt-3 space-y-4 ${darkMode?'border-gray-700':'border-gray-200'}`}>
                  ...
                  <div className="grid grid-cols-[1fr_auto] gap-2 items-center">
                    <input type="text" placeholder="Nhóm con (cách nhau phẩy)" className={`px-3 py-2 rounded-lg text-sm border ${inputClass}`} value={formData.child_label} onChange={e=>setFormData({...formData,child_label:e.target.value})}/>
                    <div className="relative w-8 h-8 rounded-full border overflow-hidden cursor-pointer">
                ...
                <button className="w-full py-3 bg-[#0F2F55] text-white rounded-xl mt-2 hover:bg-opacity-90">Lưu</button>
              </form>
            </div>
          </div>
        )}
