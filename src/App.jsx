...
import { Save, Trash2, Plus, Search, Activity, Copy, Check, Settings, LogOut, X, Filter, Tag, Upload, Download, FileUp, Pencil, Star, Moon, Sun, LayoutGrid, List, Image as ImageIcon, RotateCcw, BarChart as ChartIcon, Palette } from 'lucide-react';
...
// Default Text Colors
const DEFAULT_LIGHT_TEXT = '#0A1A2F';
...
export default function App() {
  ...
  // Text Color State
  const [lightTextColor, setLightTextColor] = useState(DEFAULT_LIGHT_TEXT);
  const [darkTextColor, setDarkTextColor] = useState(DEFAULT_DARK_TEXT);
  ...
  const fetchData = async () => {
    try {
      ...
      const localShortcuts = JSON.parse(localStorage.getItem('local_shortcuts') || '[]').map(s => ({...s, isLocal: true}));
      setShortcuts([...serverShortcuts, ...localShortcuts]);
      setLabelColors(data.labelColors || {});
      
      // Handle Configs
      const config = data.appConfig || {};
      setServerBg(config.default_background || null);
      setLightTextColor(config.text_color_light || DEFAULT_LIGHT_TEXT);
      setDarkTextColor(config.text_color_dark || DEFAULT_DARK_TEXT);

      // Handle BG Priority: Local > Server > Null
      const localBg = localStorage.getItem('custom_bg');
      if (localBg) setBgImage(localBg); 
      else if (config.default_background) setBgImage(config.default_background);
  ...
  const handleTextColorChange = (mode, color) => {
      if (mode === 'light') {
          setLightTextColor(color);
          if (isAdmin) saveConfig('text_color_light', color);
      } else {
          setDarkTextColor(color);
          if (isAdmin) saveConfig('text_color_dark', color);
      }
  };
  ...
  const handleBgUpload = (e) => { 
      ...
      r.onload=async (ev)=>{ 
          ...
          setBgImage(b); 
          if(isAdmin){ 
              if(confirm("Đặt làm mặc định toàn hệ thống?")){ 
  ...
  const fetchInsights = async () => {
      ...
      } catch (e) { alert("Lỗi tải insights"); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const isLocalSave = !isAdmin || formData.isLocal;
    if (isLocalSave) {
        const local = JSON.parse(localStorage.getItem('local_shortcuts') || '[]');
        let newLocal;
        if (formData.id && formData.isLocal) newLocal = local.map(s => s.id === formData.id ? { ...formData, id: formData.id } : s);
        else newLocal = [{ ...formData, id: Date.now(), clicks: 0, favorite: 0 }, ...local];
        localStorage.setItem('local_shortcuts', JSON.stringify(newLocal));
  ...
  const handleDelete = async (id) => { 
    ...
    if (target && target.isLocal) {
        const local = JSON.parse(localStorage.getItem('local_shortcuts') || '[]');
        localStorage.setItem('local_shortcuts', JSON.stringify(local.filter(s => s.id !== id)));
        fetchData();
    } else if (isAdmin) {
  ...
  const handleToggleFavorite = async (id,e) => { 
    ...
    if (target && target.isLocal) {
        const local = JSON.parse(localStorage.getItem('local_shortcuts') || '[]');
        localStorage.setItem('local_shortcuts', JSON.stringify(local.map(s => s.id === id ? { ...s, favorite: s.favorite ? 0 : 1 } : s)));
        fetchData();
    } else { await fetch(`/api/favorite/${id}`,{method:'POST'}); fetchData(); }
  };

  const handleLinkClick = (id, url) => { 
     const target = shortcuts.find(s => s.id === id);
     if (!target?.isLocal) fetch(`/api/click/${id}`,{method:'POST'}); 
  ...
  // Dynamic Styles
  const bgClass = darkMode ? 'bg-gray-900' : 'bg-[#F4F4F4]'; // Text color handled by inline style
  ...
  const inputClass = darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-[#D8D8D8]'; // Removed text-white to apply dynamic
            ...
            <div className="pointer-events-auto w-full max-w-2xl mx-auto flex items-center justify-center gap-3">
              <div className="flex-1 flex items-center gap-2 min-w-0">
                 <div className="relative group w-full max-w-lg mx-auto transition-all">
                    ...
                    <input type="text" className={`block w-full pl-10 pr-3 py-2 border rounded-full text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-[#009FB8] ${inputClass} ${bgImage ? 'bg-opacity-80 backdrop-blur-md' : ''}`} 
                  ...
                  <div key={item.id} className="group relative flex flex-col items-center w-28 cursor-pointer active:scale-95 transition-transform" onClick={() => handleLinkClick(item.id, item.url)}>
                    <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 scale-90">
                       ...
                       {(isAdmin || item.isLocal) && (
                        <>
                          <button onClick={e => handleEdit(item, e)} className={`p-1.5 rounded-full shadow-sm border ml-1 ${cardClass}`}><Pencil size={12}/></button>
                          <button onClick={e => { e.stopPropagation(); handleDelete(item.id); }} className={`p-1.5 rounded-full shadow-sm border ml-1 ${cardClass}`}><Trash2 size={12}/></button>
                        </>
                       )}
                    </div>
                    <button onClick={(e) => handleToggleFavorite(item.id, e)} className={`absolute -top-1 -left-1 z-10 p-1 rounded-full transition-transform hover:scale-110 ${item.favorite ? 'text-yellow-400' : 'text-gray-300 opacity-0 group-hover:opacity-100'}`}><Star size={16} fill={item.favorite ? "currentColor" : "none"} /></button>
                    ...
                    <span className={`text-sm text-center truncate w-32 px-1 leading-tight font-light`} style={{ textShadow: bgImage ? '0 1px 2px rgba(0,0,0,0.5)' : 'none' }}>{item.name}</span>
                    ...
                    {item.isLocal && <div className="absolute bottom-8 right-2 bg-blue-500/80 text-white text-[8px] px-1 rounded">LOCAL</div>}
                  </div>
                ))}
                
                <div className="flex flex-col items-center w-28 cursor-pointer group" onClick={() => { resetForm(); setShowAddModal(true); }}>
                    <div className={`w-20 h-20 rounded-[1.4rem] border-2 border-dashed flex items-center justify-center transition-all mb-3 ${darkMode ? 'border-gray-600 bg-gray-800/50' : 'border-gray-300 bg-white/50 hover:border-[#009FB8]'}`}><Plus size={32} className="opacity-50"/></div>
                    ...
                    {isAdmin ? (
                        <div className={`flex items-center gap-2 px-2 py-1 rounded-full border shadow-lg ${modalClass} bg-opacity-90 backdrop-blur`}>
                           ...
                           <div className="flex flex-col gap-1 mr-1">
                               <div className="flex items-center gap-1" title="Màu chữ Light Mode">
                                   <Sun size={10} className="text-orange-400"/>
                                   <input type="color" value={lightTextColor} onChange={(e)=>handleTextColorChange('light', e.target.value)} className="w-5 h-5 rounded cursor-pointer border-none"/>
                               </div>
                               <div className="flex items-center gap-1" title="Màu chữ Dark Mode">
                                   <Moon size={10} className="text-blue-300"/>
                                   <input type="color" value={darkTextColor} onChange={(e)=>handleTextColorChange('dark', e.target.value)} className="w-5 h-5 rounded cursor-pointer border-none"/>
                               </div>
                           </div>

                           <button onClick={fetchInsights} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full text-orange-500" title="Phân tích"><ChartIcon size={16}/></button>
                           ...
                           <button onClick={() => bgInputRef.current?.click()} className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full text-purple-500"><ImageIcon size={16}/></button>
            ...
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
              <div className={`rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 ${modalClass}`}>
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-xl flex items-center gap-2"><ChartIcon className="text-orange-500"/> Phân tích hành vi</h3>
                    <button onClick={() => setShowInsightsModal(false)}><X size={24}/></button>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                        <p className="text-sm opacity-70">Tổng lượt click</p>
                        <p className="text-3xl font-bold text-blue-500">{insightsData.totalClicks}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20">
                        <p className="text-sm opacity-70">Top 1 Ứng dụng</p>
                        <p className="text-xl font-bold text-purple-500 truncate">{insightsData.topApps[0]?.name || 'Chưa có'}</p>
                    </div>
                </div>

                <div className="space-y-6">
                    <div className="h-64 w-full p-4 rounded-xl border border-gray-500/20">
                        <h4 className="text-sm font-bold mb-4 opacity-80">Xu hướng Click (7 ngày)</h4>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={insightsData.timeline}>
                                <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
                                <XAxis dataKey="date" style={{fontSize: 10}} />
                                <YAxis style={{fontSize: 10}} />
                                <Tooltip contentStyle={{backgroundColor: darkMode?'#1f2937':'#fff', borderRadius: 8}} />
                                <Line type="monotone" dataKey="count" stroke="#009FB8" strokeWidth={3} dot={{r:4}} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="h-64 p-4 rounded-xl border border-gray-500/20">
                            <h4 className="text-sm font-bold mb-4 opacity-80">Top 5 Ứng dụng</h4>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart layout="vertical" data={insightsData.topApps}>
                                    <CartesianGrid strokeDasharray="3 3" opacity={0.1} horizontal={false}/>
                                    <XAxis type="number" hide />
                                    <YAxis dataKey="name" type="category" width={100} style={{fontSize: 10}} />
                                    <Tooltip cursor={{fill: 'transparent'}} contentStyle={{backgroundColor: darkMode?'#1f2937':'#fff', borderRadius: 8}} />
                                    <Bar dataKey="count" fill="#8884d8" radius={[0, 4, 4, 0]} barSize={20} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                        <div className="h-64 p-4 rounded-xl border border-gray-500/20">
                            <h4 className="text-sm font-bold mb-4 opacity-80">Phân bổ theo giờ</h4>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={insightsData.hourly}>
