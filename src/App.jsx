...
import { Save, Trash2, Plus, Search, Activity, Copy, Check, Settings, LogOut, X, Filter, Tag, Upload, Download, FileUp, Pencil, Star, Moon, Sun, LayoutGrid, List, Image as ImageIcon, RotateCcw, BarChart as ChartIcon, Palette, Type, RefreshCw } from 'lucide-react';

const COLOR_PRESETS = ['#0A1A2F', '#009FB8', '#6D28D9', '#BE123C', '#059669', '#C2410C', '#475569'];
const DEFAULT_LIGHT_TEXT = '#2C2C2C';
const DEFAULT_DARK_TEXT = '#E2E8F0';
...
const isVideoFile = (s) => typeof s === 'string' && /\.(mp4|webm|ogg)(\?|$)/i.test(s);
...
export default function App() {
    ...
    const [darkTextColor, setDarkTextColor] = useState(() => localStorage.getItem('custom_text_dark') || DEFAULT_DARK_TEXT);
    ...
    const [tenant, setTenant] = useState(() => normalizeTenant(localStorage.getItem('tenant')));
    ...
    useEffect(() => {
        setCurrentPage(0);
    }, [searchTerm, activeParentFilter, activeChildFilter, sortBy, tenant]);

    // --- BG LOGIC ---
    const applyBackgroundSource = (src, isFromServer = false) => {
        if (isFromServer) setServerBg(src);
        if (!src) { setBgImage(null); setBgVideo(null); return; }
        if (typeof src === 'string' && (src.startsWith('data:video') || isVideoFile(src))) {
             setBgVideo(src); setBgImage(null);
        } else {
             setBgImage(src); setBgVideo(null);
        }
    };
    ...
    const fetchData = async () => {
        try {
            ...
            // Merge Local Items
            const ls = JSON.parse(localStorage.getItem('local_shortcuts') || '[]').map(s => ({ ...s, isLocal: true, child_label: (s.child_label || '').includes('Personal') ? s.child_label : (s.child_label ? (s.child_label + ', Personal') : 'Personal') }));
            setShortcuts([...ss, ...ls]);
            ...
            if (serverVer > localVer) {
                ...
                const srvDark = c.dark_mode_default === '1';
                setDarkMode(srvDark);
            } else {
                // Normal Load
                ...
                else setDarkMode(c.dark_mode_default === '1');
            }
    ...
    const handleBgUpload = e => {
        ...
        r.onload = ev => {
            ...
            if (f.type.startsWith('video/')) { setBgVideo(b); setBgImage(null); }
            else { setBgImage(b); setBgVideo(null); }

            if (isAdmin) {
                if (confirm("Lưu mặc định server (sẽ thay thế nền/video cũ)?")) { saveConfig('default_background', b); alert("Đã lưu server!"); }
    ...
    const applyBgUrl = () => {
        ...
        if (isAdmin && confirm("Lưu làm nền mặc định server?")) { saveConfig('default_background', url); alert("Đã lưu!"); }
    ...
    const handleClearMedia = async () => {
        if (!confirm("Xóa TẤT CẢ nền ảnh/video đã tải lên?\n- Máy bạn sẽ mất nền\n- Nếu là admin: xóa luôn nền server")) return;
        localStorage.removeItem('custom_bg');
        setBgImage(null); setBgVideo(null);
        if (isAdmin) {
            try { await saveConfig('default_background', ''); alert('Đã xóa nền server và máy bạn.'); } catch { alert('Lỗi xóa server.'); }
        } else { alert('Đã xóa nền máy bạn.'); }
    };
    ...
    const handleForceSync = async () => {
        ...
        if (confirm("Cập nhật cấu hình lên Server và ép Client tải lại?")) {
            try {
                ...
                // Only sync BG if active
                const currentBg = bgVideo || bgImage;
                ...
                if (d.success) {
                    ...
                    if (clientOrder.length) {
                        ...
                        serverList = [...serverList].sort((a, b) => {
                         ...
                         // Default Sort for first sync if no custom order
    ...
    const handleSubmit = async e => {
        ...
        // AUTO FAVICON
        if (!iconToSave) {
            try { const urlObj = new URL(formData.url); iconToSave = `https://www.google.com/s2/favicons?domain=${urlObj.hostname}&sz=128`; } catch {}
        }
        ...
        if (isLocal) {
            ...
            fetchData(); setShowAddModal(false); resetForm();
        } else {
            try {
                const r = await fetch(formData.id ? `/api/shortcuts/${formData.id}` : '/api/shortcuts', { method: formData.id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                if (r.ok) { await fetchData(); setShowAddModal(false); resetForm(); }
    ...
    const handleDelete = async id => {
        ...
        if (t && t.isLocal) {
            ...
            await fetch(`/api/shortcuts/${id}`, { method: 'DELETE' }); fetchData();
        }
    };
    ...
    const handleLinkClick = (id, url) => { const t = shortcuts.find(s => s.id === id); if (!t?.isLocal) fetch(`/api/click/${id}`, { method: 'POST' }); window.open(url, '_blank'); };
    ...
    const filteredShortcuts = useMemo(() => {
        let r = shortcuts.filter(i => {
            ...
            if (!m) return false;
            if (activeChildFilter) { const tags = (i.child_label || '').split(',').map(s => s.trim()); if (!tags.includes(activeChildFilter)) return false; }
            return true;
        });
    ...
    useEffect(() => {
        const calc = () => {
            ...
            const h = cardEl ? cardEl.getBoundingClientRect().height : 140;
            const wrapRect = gridWrapperRef.current.getBoundingClientRect();
            const avail = window.innerHeight - wrapRect.top - 80;
    ...
    const currentTextColor = darkMode ? darkTextColor : lightTextColor;
    ...
    const modalClass = darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-[#D8D8D8]';
                        ...
                        <div className="flex items-center gap-2">{(bgImage || bgVideo) && <div className="flex items-center gap-1 mr-2 bg-black/40 rounded-full px-2 py-1 backdrop-blur-sm"><span className="text-[10px] text-white/90 font-bold">BG</span><input type="range" min="0" max="0.9" step="0.1" value={overlayOpacity} onChange={e => { const v = parseFloat(e.target.value); setOverlayOpacity(v); localStorage.setItem('overlayOpacity', v); if (isAdmin) saveConfig('overlay_opacity', v) }} className="w-16 h-1 accent-[#009FB8] cursor-pointer" /></div>}
                            ...
                            <div className={`flex items-center gap-1 p-1 rounded-full border shadow-lg ${inputClass} bg-opacity-80 backdrop-blur`}>
                                ...
                                <div className="hidden sm:flex items-center gap-1 ml-1">
                                    <input type="text" placeholder="Link ảnh/GIF (https://...)" className={`px-2 py-1 text-[11px] rounded-full border max-w-[160px] ${inputClass}`} value={bgUrlInput} onChange={e => setBgUrlInput(e.target.value)} />
                    ...
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
                        <div className={`rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 ${modalClass}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20"><p className="text-sm opacity-70">Tổng Click</p><p className="text-3xl font-bold text-blue-500">{insightsData.totalClicks}</p></div>
                                <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20"><p className="text-sm opacity-70">Top 1 App</p><p className="text-xl font-bold text-purple-500 truncate">{insightsData.topApps[0]?.name || 'N/A'}</p></div>
                            </div>
                                    ...
                                    <div className="flex items-end gap-1 h-32">
                                        {insightsData.timeline.map((d, i) => (
                                            <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
                                                <div className="w-full bg-emerald-400/60 rounded-t hover:bg-emerald-500 transition-all relative group" style={{ height: `${Math.max((d.count / Math.max(...insightsData.timeline.map(x => x.count), 1)) * 100, 5)}%` }}>
                                                    <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-black text-white text-[9px] px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{d.count} clicks</div>
                                                </div>
                ...
                {showAddModal && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className={`rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto border ${modalClass}`}><div className="flex justify-between items-center mb-4"><h3 className="font-bold">{formData.id ? 'Sửa' : 'Thêm'} App</h3><button onClick={() => setShowAddModal(false)}><X size={20} /></button></div><form onSubmit={handleSubmit} className="space-y-3"><input type="text" placeholder="Tên" className={`w-full px-4 py-3 rounded-xl text-sm border ${inputClass}`} value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required />
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-[10px] text-gray-400 ml-1">URL cần bắt đầu bằng https://</p>
                        </div>
