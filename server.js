...
// REORDER API
app.post('/api/reorder',(q,s)=>{
  try {
    const { order, tenant } = q.body || {};
    const ten = normalizeTenant(tenant);
    if (!Array.isArray(order)) return s.status(400).json({error:'Invalid order'});
