...
# --- 1. SERVER CODE (Node.js + Better-SQLite3) ---
server_js_content = """
...
# --- 2. FRONTEND CODE (React - Fetch API instead of SQL.js) ---
app_jsx_content = """import React, { useState, useEffect, useRef } from 'react';
...
files = {
    # 1. Package.json (Added Backend Dependencies)
    f"{PROJECT_NAME}/package.json": json.dumps({
        "name": "shortcut-manager-server",
        "private": True,
        "version": "1.0.0",
        "type": "module",
        "scripts": {
    ...
    # 2. Vite Config (Added Proxy for API Dev)
    f"{PROJECT_NAME}/vite.config.js": f"""import {{ defineConfig }} from 'vite'
...
""",
    ...
    # 6. Dockerfile (Now runs Node server instead of Nginx)
...
""",
    ...
    # 7. Docker Compose (With Volume Persistence)
...
def create_project():
    print(f"🔄 Đang tạo dự án FULL-STACK tại: {PROJECT_NAME}...")
    ...
    print("\n" + "="*50)
    print("🎉 ĐÃ CHUYỂN SANG MÔ HÌNH CLIENT-SERVER (SQLITE FILE)!")
    print("="*50)
    print("\n📦 CẤU TRÚC MỚI:")
    print("  - Backend: Node.js (Express) + Better-SQLite3")
    print("  - Database: File 'shortcuts.db' nằm trong thư mục '/data'")
    print("  - Persistence: Docker Volume map thư mục '/data' ra ngoài máy thật.")

    print("\n🚀 HƯỚNG DẪN CHẠY:")
    print("Cách 1: Chạy Docker (Khuyên dùng):")
    print(f"  1. cd {PROJECT_NAME}")
    print("  2. docker-compose up -d --build")
    print(f"  -> App chạy tại: http://localhost:{PORT}")
    print("  -> Dữ liệu lưu tại thư mục: ./data (trên máy của bạn)")

    print("\nCách 2: Chạy Dev (Thủ công):")
    print("  1. npm install")
    print("  2. npm run dev (Frontend)")
    print("  3. node server.js (Backend - Cần mở terminal khác)")
