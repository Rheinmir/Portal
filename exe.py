...
app.get('/api/insights',(q,s)=>{try{
  const tc=Number(db.prepare('SELECT SUM(clicks) AS count FROM shortcuts').get().count||0);
  const top=db.prepare('SELECT name,clicks AS count FROM shortcuts ORDER BY count DESC LIMIT 10').all();top.forEach(a=>a.count=Number(a.count||0));
  const timeline=db.prepare(`SELECT DATE(clicked_at) AS d,COUNT(*) AS count FROM click_logs WHERE clicked_at >= DATE('now','-7 day') GROUP BY DATE(clicked_at) ORDER BY d ASC`).all();timeline.forEach(t=>t.count=Number(t.count||0));
  s.json({totalClicks:tc,topApps:top,timeline});
}catch(e){s.status(500).json({error:e.message})}});
...
# --- 2. FRONTEND CODE ---
app_jsx_content = """import React,{useState,useEffect,useRef,useMemo}from'react';import{Save,Trash2,Plus,Search,Activity,Copy,Check,Settings,LogOut,X,Filter,Tag,Upload,Download,FileUp,Pencil,Star,Moon,Sun,LayoutGrid,List,Image as ImageIcon,RotateCcw,BarChart as ChartIcon,Palette,Type}from'lucide-react';
